...
import { emitHeritageClauses, emitSingleClass, emitSingleInterface } from "./emitter";
import { findFilesByGlob } from "./io";
import { getAst, parseClasses, parseHeritageClauses, parseInterfaces } from "./parser";

export async function getDsl(tsConfigPath: string, pattern: string) {
  const sourceFilesPaths = await findFilesByGlob(pattern);

  console.log(chalk.yellowBright("Matched files:\n" + sourceFilesPaths.reduce((p, c) => `${p}${c}\n`, "")));
